...
export function InputBar({ config, skills, cwd }) {
    ...
    const runLoop = useCallback(async (contextMsgs, depth = 0) => {
        ...
        await stream({
            ...
            async onDone() {
                ...
                // Commit streaming response to terminal scrollback
    ...
    const handleSubmit = useCallback(async (text) => {
        ...
        if (text.startsWith('/')) {
            ...
            printer.systemMsg(`unknown skill: /${cmd}. Try /list`);
            return;
        }
        // Expand @file refs into LLM context
