 ...
 */
export function compactContext(messages: ChatMessage[], goal?: string): ChatMessage[] {
  ...
  const parts: string[] = [`[context compacted — ${middle.length} messages summarised]`]
  if (goal) parts.push(`Goal: ${goal}`)
  if (toolResults.length) parts.push(`Completed:\n${toolResults.join('\n')}`)
  if (assistantSummaries.length) parts.push(`Last reasoning: ${assistantSummaries.at(-1)}`)

  const summary: ChatMessage = { role: 'user', content: parts.join('\n\n') }
