...
export const grep: Tool<Input> = {
  name: 'grep',
  description: 'Search file contents by regex. Prefer this over grep via run_bash: results are capped and formatted.',
  input_schema: {
    type: 'object',
    properties: {
      pattern:          { type: 'string', description: 'Regex (literal when fixed_strings)' },
      ...
      files_only:       { type: 'boolean', description: 'Filenames only' },
      type:             { type: 'string', description: 'ripgrep type filter, e.g. "js"' },
      multiline:        { type: 'boolean', description: 'Allow matches to span lines' },
      count:            { type: 'boolean', description: 'Count matches per file' },
      fixed_strings:    { type: 'boolean', description: 'Literal string, not regex' },
    },
    required: ['pattern'],
  },
  handler: async ({
